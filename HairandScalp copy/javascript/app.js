...
$scope.main = [
			...
			{
				bool: false,
				head: 'What statement best describes your hair loss?',
			...
			{
				bool: false,
				head: 'Based on your answers, these treatment options are best suited to your needs:',
			...
			},
			{
				...
				para: 'Hair enhancement is an option that integrates real human hair using a process that is non-surgical and non-invasive. Clients who have male pattern baldness, or women who have lost a substantial amount of hair due to genetics or health conditions causing permanent baldness, can benefit from our hair enhancement services. The grafting procedure is virtually undetectable. The style, color and texture is matched to the client’s existing hair and blended to create a fuller and thicker appearance. We have helped change thousands of client’s lives over the past 48 years with our hair enhancement services.',
	...
	// $scope.wheatfromchaff = function(arg) {
	// 			if (main[counter].arg = null) {
	//
	// 			}
	// 	};
	...
	$scope.changebool = function(obj) {
		...
		$scope.invis = $scope.main[0];
		console.log($scope.invis);

	};

	// $scope.somefunction = function(thing) {
	// 	var showthing = false;
	// 	if (thing == true) {
	// 	// 	showthing = true;
	// 	var showthing = true;
